...
function scanAreaForLootSpawn(tx, ty, items, loot, target) {
    ...
    while (scanedArea.length < items) {
        for (let i = 0; i < area_d; i++) {
            for (let j = 0; j < area_d; j++) {
                if (
                    typeof cmArr[tx / ts - Math.floor(area_d / 2) + i][
                        ty / ts - Math.floor(area_d / 2) + j
                    ] === 'undefined' &&
                    typeof smArr[tx / ts - Math.floor(area_d / 2) + i][
                        ty / ts - Math.floor(area_d / 2) + j
                    ] != 'undefined'
                ) {
                    if (lootArr.length > 0) {
                        for (let l = 0; l < lootArr.length; l++) {
                            if (
                                lootArr[l].x ==
                                    (tx / ts - Math.floor(area_d / 2) + i) *
                                        ts &&
                                lootArr[l].y ==
                                    (ty / ts - Math.floor(area_d / 2) + j) * ts
                            ) {
                                spawnPermit = false;
                            }
                        }
    ...
    while (places.length < items) {
        ...
        for (let i = 0; i < places.length; i++) {
            if (
                places[i].x == scanedArea[gen].x &&
                places[i].y == scanedArea[gen].y
            ) {
                getPlace = false;
            }
        }
        if (getPlace) places.push(scanedArea[gen]);
    }

    for (let i = 0; i < places.length; i++) {
        let lootSprite = new Chest(
            places[i].x / target.tile_size.w,
            places[i].y / target.tile_size.h,
            3,
            loot[i].lootIcon,
            'loot',
            loot[i]
...
function updateMiniMap() {
    ...
    for (let i = 0; i < all_sprites.length; i++) {
        if (all_sprites[i].name && all_sprites[i].state != 0) {
            if (
                all_sprites[i].name &&
                all_sprites[i].name == 'floor' &&
                all_sprites[i].state == 2
            ) {
                c.fillStyle = '#fff';
                c.fillRect(
                    (all_sprites[i].x / 32) * scale,
                    (all_sprites[i].y / 32) * scale,
                    32 / scale,
                    32 / scale
                );
            }
            ...
            if (
                all_sprites[i].name == 'door_closed' ||
                all_sprites[i].name == 'door_opened'
            ) {
                c.fillStyle = '#893a02';
                c.fillRect(
                    (all_sprites[i].x / 32) * scale,
...
Dungeon = {
    ...
    _generateMap: function () {
        var map = new ROT.Map.Digger(this.map_size, this.map_size, {
            roomWidth: [4, 6],
            roomHeight: [4, 6],
            timeLimit: 4000,
        });
